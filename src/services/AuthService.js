...
class AuthService {
   ...
   */
  async register(username, email, password, phone, captchaToken = null, termsAccepted = false) {
    try {
   ...
   */
  async login(identifier, password, rememberMe = true, captchaToken = null, confirmOverride = false, portalType = 'USER') {
    try {
      ...
      // Store token and user info in local storage
      if (response.data.token && !response.data.sessionConflict) {
        this.setSession(response.data);
      }
  ...
  async loginWithGoogle(credential, confirmOverride = false, portalType = 'USER') {
    try {
   ...
   */
  async loginWithOtp(phone, otpCode, confirmOverride = false, portalType = 'USER') {
    try {
