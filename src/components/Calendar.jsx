...
const Calendar = ({ events = [] }) => {
  ...
  const renderCells = () => {
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        days.push(
          <div
            className={`p-2 text-center rounded cursor-pointer border ${!isSameMonth(day, monthStart) ? 'text-muted-foreground' : isSameDay(day, selectedDate) ? 'bg-indigo-900/50 text-indigo-300' : ''} ${hasEvent ? 'border-indigo-500' : 'border-transparent'}`}
            key={day}
  ...
  return (
    <div className="bg-muted border border-border rounded-lg shadow-lg p-4 w-full max-w-xl mx-auto text-white">
      {renderHeader()}
      {renderDays()}
      {renderCells()}
      {renderEventList()}
