    ...
    if (part.type === 'link') {
      ...
      return (
        <a
          ...
          className="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-semibold underline underline-offset-2 hover:text-emerald-500 transition-colors"
        >
          <span>{part.text}</span>
        ...
        {tokens.map((token, tIdx) => {
          ...
          if (token.startsWith('`') && token.endsWith('`') && token.length > 2) {
            return (
              <code
                key={`code-${tIdx}`}
                className="px-1.5 py-0.5 rounded-md bg-muted font-mono text-xs text-emerald-700 dark:text-emerald-300 border border-border"
              >
                {token.slice(1, -1)}
              </code>
            )
...
function CodeBlock({ code, language }) {
  ...
  return (
    <div className="relative my-4 rounded-2xl overflow-hidden border border-border bg-slate-950 text-slate-100 shadow-md">
      <div className="flex items-center justify-between px-4 py-2 bg-slate-900 border-b border-border/40 text-xs text-muted-foreground font-mono">
        <span>{language || 'code'}</span>
        <button
          type="button"
          onClick={handleCopy}
          className="flex items-center gap-1.5 text-xs text-slate-300 hover:text-white transition cursor-pointer"
        >
          {copied ? (
            <>
...
function CalloutBox({ type, text }) {
  const configs = {
    ...
    important: {
      ...
      icon: <ShieldCheck className="h-5 w-5 text-purple-600 dark:text-purple-400 shrink-0 mt-0.5" />,
      title: 'Key Takeaway',
    },
    info: {
  ...
  return (
    <div className={`my-5 p-4 sm:p-5 rounded-2xl border ${current.border} flex items-start gap-3.5 shadow-xs`}>
      {current.icon}
      <div className="text-sm leading-relaxed">
        <p className="font-bold text-xs uppercase tracking-wider mb-1 opacity-90">{current.title}</p>
...
export default function RichArticleRenderer({ content, className = '' }) {
  ...
  return (
    <div className={`space-y-4 text-foreground/90 ${className}`}>
      {elements.map((block, idx) => {
        switch (block.type) {
          case 'heading': {
            ...
            const sizeClass =
              block.level === 1
                ? 'text-2xl sm:text-3xl font-black mt-8 mb-4 border-b border-border/50 pb-2 text-foreground tracking-tight'
                : block.level === 2
                ? 'text-xl sm:text-2xl font-bold mt-7 mb-3 text-foreground tracking-tight'
                : block.level === 3
                ? 'text-lg sm:text-xl font-bold mt-6 mb-2 text-foreground'
                : 'text-base sm:text-lg font-bold mt-5 mb-2 text-foreground'

            return (
              <HeadingTag
                key={`h-${idx}`}
                id={block.id}
                className={`scroll-mt-24 group flex items-center justify-between ${sizeClass}`}
              >
                <span>{block.text}</span>
                <a
                  href={`#${block.id}`}
                  className="opacity-0 group-hover:opacity-100 text-xs font-normal text-emerald-600 hover:underline ml-2 transition-opacity"
          ...
          case 'paragraph':
            return (
              <p
                key={`p-${idx}`}
                className="text-base sm:text-lg leading-relaxed text-foreground/90 my-3 font-normal"
              >
          ...
          case 'blockquote':
            return (
              <blockquote
                key={`bq-${idx}`}
                className="my-5 pl-4 sm:pl-6 py-2 border-l-4 border-emerald-500 italic text-base sm:text-lg text-foreground/80 bg-muted/30 rounded-r-2xl"
              >
          ...
          case 'ul_item':
            return (
              <div key={`ul-${idx}`} className="flex items-start gap-2.5 my-2 pl-2">
                <span className="h-2 w-2 rounded-full bg-emerald-500 mt-2 shrink-0" />
                <span className="text-base sm:text-lg leading-relaxed text-foreground/90">
                  {renderInlineFormatting(block.text)}
                </span>
              </div>
            )
          ...
          case 'ol_item':
            return (
              <div key={`ol-${idx}`} className="flex items-start gap-3 my-2 pl-2">
                <span className="h-6 w-6 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                  {block.number}
                </span>
                <span className="text-base sm:text-lg leading-relaxed text-foreground/90">
                  {renderInlineFormatting(block.text)}
          ...
          case 'image':
            return (
              <figure key={`img-${idx}`} className="my-6">
                <div
                  className="relative group rounded-2xl overflow-hidden border border-border shadow-md bg-muted/40 cursor-pointer"
                ...
                >
                  <img
                    ...
                    loading="lazy"
                    className="w-full max-h-[480px] object-cover transition duration-300 group-hover:scale-101"
                    onError={(e) => {
          ...
          case 'hr':
            return <hr key={`hr-${idx}`} className="my-8 border-border/60" />
      ...
      {lightboxImage && (
        <div
          className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn"
          onClick={() => setLightboxImage(null)}
        >
          <button
            type="button"
            onClick={() => setLightboxImage(null)}
            className="absolute top-5 right-5 h-10 w-10 rounded-full bg-white/20 text-white flex items-center justify-center hover:bg-white/30 transition cursor-pointer"
