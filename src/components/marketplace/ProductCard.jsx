...
export function ProductCard({
  ...
  const currentUserId = typeof window !== 'undefined' ? localStorage.getItem('farmEazy_userId') : null;
  const isOwnProduct = currentUserId && product.sellerId != null && String(currentUserId) === String(product.sellerId);
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-end justify-between gap-2 pt-1">
          ...
          <div className="text-right">
            {product.pricingType === 'BIDDING' ? (
              <div>
                <span className="text-lg font-semibold text-primary">Bidding</span>
                <span className="text-xs text-muted-foreground block">
                  {product.bidCloseAt ? `Closes ${new Date(product.bidCloseAt).toLocaleDateString()}` : ''}
                </span>
              </div>
            ) : (
              <>
        ...
        <div className="flex gap-2">
          <Button
            ...
            size="sm"
            disabled={notDeliverable || outOfStock || isOwnProduct}
            onClick={() => onAddToCart?.(product)}
          >
            <ShoppingCart className="h-4 w-4" />
            {isOwnProduct ? 'Your listing' : notDeliverable ? 'Not deliverable' : 'Add to cart'}
          </Button>
          <Button
            type="button"
            size="sm"
