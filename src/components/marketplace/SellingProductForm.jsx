...
export function SellingProductForm({ editingProduct: initialProduct, onClose, onSuccess }) {
  ...
  const [formData, setFormData] = useState({
    ...
    specifications: '',
    warrantyInfo: '',
    livestockType: '', livestockBreed: '', livestockSex: '', livestockAge: '', livestockWeight: '', livestockHealthStatus: '', livestockVaccinationStatus: '', livestockTransportNotes: '',
    imageUrls: '',
    videoUrls: '',
    contactEmail: '',
    contactPhone: '',
  ...
  const handleEditProduct = (product) => {
    ...
    setFormData({
      ...
      livestockType: product.livestockType || '', livestockBreed: product.livestockBreed || '', livestockSex: product.livestockSex || '', livestockAge: product.livestockAge || '', livestockWeight: product.livestockWeight || '', livestockHealthStatus: product.livestockHealthStatus || '', livestockVaccinationStatus: product.livestockVaccinationStatus || '', livestockTransportNotes: product.livestockTransportNotes || '',
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.vendorName || !formData.vendorLocation || !formData.vendorType) {
      // Vendor identity/location are derived server-side from the authenticated account.
      // The frontend no longer requires the vendor to manually type them.
    }

    if (!formData.contactEmail || !formData.contactPhone) {
      // Contact email/phone are derived server-side from the authenticated account.
    }

    const publishConsent = window.confirm(
      `Confirm product listing details:\n\nProduct: ${formData.productName}\nCategory: ${formData.category}\nPricing: ${formData.pricingType}\nAvailability: ${formData.locationScope}${formData.locationState ? ' / ' + formData.locationState : ''}${formData.locationPincode ? ' / ' + formData.locationPincode : ''}\nQuantity: ${formData.quantity} ${formData.unit}\n\nProceed to publish?`
    );
    if (!publishConsent) {
      return;
    }
  ...
  useEffect(() => {
    if (initialProduct) {
      ...
      setFormData({
        ...
        livestockType: initialProduct.livestockType || '', livestockBreed: initialProduct.livestockBreed || '', livestockSex: initialProduct.livestockSex || '', livestockAge: initialProduct.livestockAge || '', livestockWeight: initialProduct.livestockWeight || '', livestockHealthStatus: initialProduct.livestockHealthStatus || '', livestockVaccinationStatus: initialProduct.livestockVaccinationStatus || '', livestockTransportNotes: initialProduct.livestockTransportNotes || '',
            ...
            {currentStep === 2 && (
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <FormField label="Pricing mode" id="pricingType" hint="Bidding hides your asking price; buyers place bids within a 24-hour window.">
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        <Button
                          key={opt.value}
                          type="button"
                          variant={formData.pricingType === opt.value ? 'default' : 'outline'}
                          className="h-auto py-2.5 text-xs"
                  ...
                  <FormField label="Availability scope" id="locationScope" hint="Who can see this listing based on location. Select from the pincode master — location is enforced server-side.">
                    <LocationScopePicker
                      selectClass={selectClass}
                      value={{ scope: formData.locationScope, state: formData.locationState, pincode: formData.locationPincode }}
                  ...
                  {formData.pricingType === 'BIDDING' && (
                    <FormField label="Minimum bidding quantity" id="minBidQuantity">
                      <input
                        id="minBidQuantity"
                        name="minBidQuantity"
                        type="number"
                        min="1"
                        value={formData.minBidQuantity}
                        onChange={handleInputChange}
                        className={selectClass}
                      />
                    </FormField>
                ...
                <CardContent className="space-y-6">
                  {formData.pricingType === 'BIDDING' ? (
                    <div className="p-4 rounded-lg border border-border bg-muted/40 text-sm">
                      Bidding listing — no asking price shown to buyers. Buyers will place bids
                      within a 24-hour window after listing. You can review and accept the best bid.
                    </div>
                  ) : (
