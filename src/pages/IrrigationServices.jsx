...
import LocationScopePicker from '../components/location/LocationScopePicker'
import ServiceBidPanel from '../components/marketplace/ServiceBidPanel'
...
function IrrigationServices() {
  ...
  useEffect(() => {
    ...
    // Only refresh listings on a genuine cross-tab location change. Ignore
    // generic storage writes (farmEazy_lastSyncAt / farmEazy_lastActivity) so
    // API responses never create a cross-tab fetch loop.
    const onStorage = (event) => {
      if (event.key === 'farmeazy_selected_location') {
        onLocationChanged()
      }
    }
  ...
  const handlePostSubmit = async (e) => {
    e.preventDefault();
    if (!postForm.title) {
      showToast('Service title is required', 'warning')
      return
    }
    if (postForm.pricingType === 'FIXED_PRICE' && (!postForm.rate || Number(postForm.rate) <= 0)) {
      showToast('Please enter a price for fixed-price listings', 'warning')
      return
    }
    if (postForm.locationScope === 'STATE' && !postForm.locationState) {
      showToast('Please select a state for state-scoped listings', 'warning')
      return
    }
    if (postForm.locationScope === 'PINCODE' && !postForm.locationPincode) {
      showToast('Please select a pincode for pincode-scoped listings', 'warning')
      return
    }

    const confirmation = window.confirm(
      `Confirm service listing details:\n\nService: ${postForm.title}\nLocation scope: ${postForm.locationScope}\nRate: ${postForm.pricingType === 'BIDDING' ? 'Bidding (hidden)' : 'INR ' + postForm.rate + '/hour'}\n\nProceed to publish?`
    );
    if (!confirmation) {
      return;
    }
    ...
    try {
      ...
      const serviceData = {
        ...
        description: `${postForm.type} service. Location scope: ${postForm.locationScope}${postForm.locationState ? ' / ' + postForm.locationState : ''}${postForm.locationPincode ? ' / ' + postForm.locationPincode : ''}. Contact: ${postForm.contactName || 'N/A'}, Phone: ${postForm.contactPhone || 'N/A'}. Status: ${postForm.availability}`,
        price: postForm.pricingType === 'BIDDING' ? null : (postForm.rate ? parseFloat(postForm.rate) : 0),
        type: postForm.type,
        location: postForm.locationScope === 'PINCODE' ? (postForm.locationPincode || '') : (postForm.locationState || 'India'),
        ...
        pricingType: postForm.pricingType,
        locationScope: postForm.locationScope,
        locationState: postForm.locationState,
        locationPincode: postForm.locationPincode,
        minBidQuantity: postForm.minBidQuantity
      };
                  ...
                  <div className="bg-muted rounded-xl p-6 border border-border">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <LocationScopePicker
                          value={{ scope: postForm.locationScope, state: postForm.locationState, pincode: postForm.locationPincode }}
                          onChange={(loc) => setPostForm((p) => ({
                            ...p,
                        ...
                        />
                        <p className="text-xs text-muted-foreground mt-1">Location scope determines who can see this listing (enforced server-side).</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-muted-foreground mb-2">
                          Pricing Mode
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <button
                            type="button"
                            onClick={() => setPostForm((p) => ({ ...p, pricingType: 'FIXED_PRICE' }))}
                            className={`px-3 py-2 rounded-md text-sm border ${postForm.pricingType === 'FIXED_PRICE' ? 'border-primary bg-primary/10 text-primary' : 'border-border'}`}
                          >
                            Fixed Price
                          </button>
                          <button
                            type="button"
                            onClick={() => setPostForm((p) => ({ ...p, pricingType: 'BIDDING' }))}
                            className={`px-3 py-2 rounded-md text-sm border ${postForm.pricingType === 'BIDDING' ? 'border-primary bg-primary/10 text-primary' : 'border-border'}`}
                          >
                            Bidding
                          </button>
                        </div>
                      </div>
                      {postForm.pricingType === 'FIXED_PRICE' ? (
                        <div>
                          <label className="block text-sm font-semibold text-muted-foreground mb-2">
                          ...
                          <div className="relative">
                            <input
                              ...
                              className="form-input pl-10 hover:border-purple-400 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 transition-all"
                              placeholder="1200"
                              required={postForm.pricingType === 'FIXED_PRICE'}
                            />
                            <div className="absolute left-3 top-1/2 transform -translate-y-1/2 pointer-events-none">
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <input
                              ...
                              type="number"
                              min="1"
                              value={postForm.minBidQuantity}
                              onChange={handlePostChange}
                              className="form-input pl-10 hover:border-purple-400 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 transition-all"
                            />
                          </div>
                          <p className="text-xs text-muted-foreground mt-1">
                            Buyers place bids within a 24-hour window; your asking price stays hidden.
                          </p>
                        </div>
                      )}
                      <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {listings.map((listing) => (
                  <div key={listing.id} className={`rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow border ${isDark ? 'bg-muted border-border' : 'bg-background border-border'}`}>
                    ...
                    <div className={`space-y-2 mb-4 pb-4 border-b ${isDark ? 'border-border' : 'border-border'}`}>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        {listing.pricingType === 'BIDDING' ? (
                          <span className="text-xs px-2 py-1 rounded-full font-semibold bg-purple-100 text-purple-700">Bidding</span>
                        ) : (
                    ...
                    {listing.pricingType === 'BIDDING' && (
                      <button
                        onClick={() => navigate(`/selling/bids/service/${listing.id}`)}
                        className="mt-2 w-full bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-lg text-sm font-semibold transition"
                      >
                        ⚖️ View Bids
                      </button>
                    )}
                  ...
                  <div key={listing.id} className={`rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow border-2 ${isDark ? 'bg-muted border-border' : 'bg-background border-border'}`}>
                    ...
                    <div className={`space-y-2 mb-4 pb-4 border-b ${isDark ? 'border-border' : 'border-border'}`}>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        {listing.pricingType === 'BIDDING' ? (
                          <p className="text-lg font-bold text-purple-600">Bidding</p>
                        ) : (
                    ...
                    {listing.pricingType === 'BIDDING' ? (
                      <ServiceBidPanel listing={listing} />
                    ) : (
                      <button
