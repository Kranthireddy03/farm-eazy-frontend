...
function Home() {
  ...
  useEffect(() => {
    ...
    // Only react to storage keys that genuinely change Home data (cross-tab
    // location or role sync). Ignore generic writes such as
    // farmEazy_lastSyncAt / farmEazy_lastActivity so API responses never
    // trigger a cross-tab fetch loop.
    const onStorage = (event) => {
      if (event.key === 'farmeazy_selected_location' || event.key === 'farmEazy_roles') {
        fetchStats()
      }
    }
      ...
      <PremiumHero
        ...
        actions={
          <>
            ...
            <Button variant="outline" onClick={() => navigate('/products/listings')}>Manage listings</Button>
          </>
        }
        stats={[
