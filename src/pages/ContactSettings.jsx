...
export default function ContactSettings() {
  ...
  const loadContact = async () => {
    try {
      const resp = await apiClient.get('/account/contact');
      const updatedContact = { email: resp.data?.email || '', phone: resp.data?.phone || '' };
      setContact(updatedContact);

      if (updatedContact.email) {
        localStorage.setItem(STORAGE_KEYS.USER_EMAIL, updatedContact.email);
      }
      if (updatedContact.phone) {
        localStorage.setItem(STORAGE_KEYS.USER_PHONE, updatedContact.phone);
      }
    } catch {
