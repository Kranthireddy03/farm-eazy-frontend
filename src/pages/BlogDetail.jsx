...
export default function BlogDetail() {
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-slate-950 text-slate-100' : 'bg-slate-50/50 text-slate-900'}`}>
      ...
      <div className="fixed top-0 left-0 right-0 h-1.5 z-[100] bg-muted/40">
        <div
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 sm:py-12 space-y-8">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-28 gap-4">
            ...
            <p className="text-sm font-semibold text-muted-foreground">Opening article…</p>
          </div>
        ) : error ? (
          <div className={`rounded-3xl border p-8 text-center space-y-4 max-w-xl mx-auto ${isDark ? 'bg-red-950/20 border-red-800 text-red-200' : 'bg-red-50 border-red-200 text-red-700'}`}>
            <h2 className="text-xl font-bold">Article Unavailable</h2>
          ...
          <div className="space-y-10">
            ...
            <header className="space-y-6 max-w-4xl">
              <div className="flex items-center gap-2.5 flex-wrap">
                <Badge className="bg-emerald-600 text-white font-bold text-xs uppercase px-3 py-1 shadow-xs">
                  {post.category || 'General'}
                </Badge>
                {post.tags && Array.isArray(post.tags) && post.tags.map((t, idx) => (
                  <span
                    key={idx}
                    className="px-2.5 py-0.5 rounded-full bg-muted text-xs font-medium text-muted-foreground border border-border"
                  >
                    #{t}
                  </span>
                ))}
              ...
              <h1 className="text-3xl sm:text-5xl lg:text-5xl font-black text-foreground tracking-tight leading-[1.15]">
                {post.title}
              </h1>
              ...
              {/* Author & Meta Row */}
              <div className="pt-4 border-t border-b border-border/70 py-4 flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 text-white font-black text-lg flex items-center justify-center shadow-md">
                    {post.authorName ? post.authorName.charAt(0).toUpperCase() : 'F'}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-bold text-sm text-foreground">{post.authorName || 'FarmEazy Editorial'}</p>
                      <Badge variant="outline" className="text-[10px] py-0 px-1.5 font-bold border-emerald-500/40 text-emerald-600 dark:text-emerald-400 bg-emerald-500/10">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`h-9 px-3.5 rounded-full border text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer ${
                      isBookmarked
                  ...
                  >
                    {copiedLink ? <CheckCircle2 className="h-4 w-4 text-white" /> : <Copy className="h-4 w-4" />}
            ...
            {/* Cover Image Banner */}
            {coverImage && (
              <div className="relative rounded-3xl overflow-hidden border border-border shadow-xl max-h-[460px] bg-muted">
                <img
                  src={coverImage}
                  alt={post.title}
            ...
            <div className="grid lg:grid-cols-[1fr_320px] gap-10 items-start">
              ...
              <main className="space-y-8 min-w-0">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {galleryImages.map((imgUrl, i) => (
                          <img
                            ...
                            loading="lazy"
                            className="w-full h-48 object-cover rounded-2xl border border-border shadow-sm hover:scale-101 transition-transform"
                ...
                <div className={`p-6 sm:p-8 rounded-3xl border shadow-md space-y-4 ${
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    {isAuthenticated ? (
                        ...
                        {[1, 2, 3, 4, 5].map((star) => {
                          ...
                          return (
                            <button
                              ...
                              className="p-1 text-2xl transition hover:scale-125 cursor-pointer disabled:opacity-60"
                      ...
                      <Button asChild size="sm" variant="outline" className="rounded-full text-xs font-semibold">
                ...
                <section className={`p-6 sm:p-8 rounded-3xl border shadow-sm space-y-6 ${
                  isDark ? 'bg-slate-900/70 border-border' : 'bg-white border-border'
                }`} id="comments">
                  <div className="flex items-center justify-between border-b border-border pb-4">
                    <div className="flex items-center gap-2.5">
                      <MessageSquare className="h-5 w-5 text-emerald-600 dark:text-emerald-400" />
                      <h3 className="text-xl font-black text-foreground">Reader Comments & Discussion</h3>
                  ...
                  {isAuthenticated ? (
                    <form onSubmit={handleCommentSubmit} className="space-y-3">
                      <div className="flex items-start gap-3">
                        <div className="h-9 w-9 rounded-full bg-emerald-600 text-white font-bold flex items-center justify-center text-xs shrink-0 mt-1">
                        ...
                        <div className="flex-1 space-y-2">
                          <textarea
                            ...
                            className={`w-full p-3.5 rounded-2xl border text-xs sm:text-sm outline-none transition ${
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              ...
                              size="sm"
                              className="rounded-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs cursor-pointer shadow-sm"
                            >
                    ...
                    <div className={`p-4 rounded-2xl border text-center space-y-2 ${isDark ? 'bg-slate-950/60 border-border' : 'bg-slate-50 border-border'}`}>
                      <p className="text-sm font-semibold text-foreground">Join the Community Discussion</p>
                      <p className="text-xs text-muted-foreground">Please login to ask questions and share insights on this article.</p>
                    ...
                    {comments.length === 0 ? (
                      ...
                      comments.map((c, idx) => (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <div className="h-7 w-7 rounded-full bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 font-bold flex items-center justify-center text-xs">
                                {(c.authorName || 'User').charAt(0).toUpperCase()}
                              </div>
                              <span className="font-bold text-xs text-foreground">{c.authorName || 'Community Member'}</span>
                            </div>
                            <span className="text-[11px] text-muted-foreground">
                          ...
                          <p className="text-xs sm:text-sm text-foreground/90 whitespace-pre-wrap leading-relaxed pl-9">
                            {c.content}
                          </p>
                        </div>
                      ))
              ...
              {/* Right Column: Sticky Table of Contents & Social Links */}
              <aside className="space-y-6 lg:sticky lg:top-20">
                ...
                <div className={`p-5 rounded-3xl border shadow-sm space-y-3 ${
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <a
                      ...
                      className="p-2.5 rounded-xl border border-border bg-muted/40 hover:bg-emerald-500/10 hover:border-emerald-500/30 text-xs font-semibold flex items-center justify-center gap-1.5 transition text-foreground"
                    >
                      <span>💬 WhatsApp</span>
                    </a>
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="p-2.5 rounded-xl border border-border bg-muted/40 hover:bg-sky-500/10 hover:border-sky-500/30 text-xs font-semibold flex items-center justify-center gap-1.5 transition text-foreground"
                    >
                      <span>🐦 Twitter (X)</span>
                    </a>
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="p-2.5 rounded-xl border border-border bg-muted/40 hover:bg-blue-500/10 hover:border-blue-500/30 text-xs font-semibold flex items-center justify-center gap-1.5 transition text-foreground"
                    >
                      <span>💼 LinkedIn</span>
                    </a>
                    <button
                      type="button"
                      onClick={handleCopyLink}
                      className="p-2.5 rounded-xl border border-border bg-muted/40 hover:bg-muted text-xs font-semibold flex items-center justify-center gap-1.5 transition text-foreground cursor-pointer"
                    >
                ...
                <div className={`p-5 rounded-3xl border shadow-sm space-y-3 ${
                  ...
                  <div className="flex items-center gap-3">
                    <div className="h-11 w-11 rounded-full bg-emerald-600 text-white font-bold text-sm flex items-center justify-center shrink-0">
                      {post.authorName ? post.authorName.charAt(0).toUpperCase() : 'F'}
                    </div>
                    <div>
                      <p className="font-bold text-sm text-foreground">{post.authorName || 'FarmEazy Editorial'}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {relatedPosts.map((rel) => (
                    <Link
                      key={rel.id}
                      to={`/blog/${rel.slug}`}
                      className={`group rounded-3xl border overflow-hidden p-5 flex flex-col justify-between transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${
                    ...
                    >
                      <div className="space-y-2.5">
                        ...
                        <h4 className="text-base font-bold text-foreground group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors line-clamp-2 leading-snug">
                          {rel.title}
                        </h4>
                      ...
                      <div className="pt-4 mt-4 border-t border-border flex items-center justify-between text-xs text-emerald-600 dark:text-emerald-400 font-bold">
