...
function Login() {
  ...
  // Conflict and confirmation modal states
  const [showConflictModal, setShowConflictModal] = useState(false);
  const [conflictType, setConflictType] = useState(null); // 'different_account' or 'session_conflict'
  const [currentUserEmail, setCurrentUserEmail] = useState('');
  const [conflictSessionDetails, setConflictSessionDetails] = useState(null);
  ...
  const handleGoogleLogin = async (credential, forceOverride = false) => {
    setApiError('')
    const googleProfile = decodeGoogleProfile(credential)
    
    // Scenario 3 check for Google Login
    if (!forceOverride && googleProfile?.email) {
      const currentEmail = localStorage.getItem('farmEazy_email');
      if (currentEmail && currentEmail.toLowerCase() !== googleProfile.email.toLowerCase()) {
        setCurrentUserEmail(currentEmail);
        setConflictType('different_account');
        setPendingLoginAction(() => () => handleGoogleLogin(credential, true));
    ...
    setLoading(true)
    try {
      const authResponse = await AuthService.loginWithGoogle(credential, forceOverride, 'USER')
      ...
      if (window.__farmeazyGoogleInitMode !== googleMode) {
        ...
        window.google.accounts.id.initialize({
          ...
          callback: async (response) => {
            ...
            await handleGoogleLogin(response.credential, false);
          },
        })
  ...
  const handlePasswordSubmit = async (e, forceOverride = false) => {
    if (e) e.preventDefault()
    setApiError('')
    if (!validatePasswordForm()) return

    // Scenario 3 check: different account already logged in
    if (!forceOverride) {
      const currentEmail = localStorage.getItem('farmEazy_email');
      const currentUsername = localStorage.getItem('farmEazy_username');
      const inputIdentifier = formData.identifier.trim().toLowerCase();
      if ((currentEmail && currentEmail.toLowerCase() !== inputIdentifier) && 
          (currentUsername && currentUsername.toLowerCase() !== inputIdentifier)) {
        if (currentEmail || currentUsername) {
          setCurrentUserEmail(currentEmail || currentUsername);
          setConflictType('different_account');
          setPendingLoginAction(() => () => handlePasswordSubmit(null, true));
          setShowConflictModal(true);
          return;
        }
      }
    ...
    try {
      ...
      const response = await AuthService.login(formData.identifier, formData.password, rememberMe, captchaToken, forceOverride, 'USER')
  ...
  const handleVerifyOtp = async (e, forceOverride = false) => {
    if (e) e.preventDefault()
    setApiError('')
    if (!validateOtpCodeForm()) return

    // Scenario 3 check for OTP login
    if (!forceOverride) {
      const currentPhone = localStorage.getItem('farmEazy_phone');
      if (currentPhone && currentPhone !== otpFormData.phone) {
        const currentEmail = localStorage.getItem('farmEazy_email');
        setCurrentUserEmail(currentEmail || 'Another User');
        setConflictType('different_account');
        setPendingLoginAction(() => () => handleVerifyOtp(null, true));
        setShowConflictModal(true);
        return;
      }
    }
    ...
    setLoading(true)
    try {
      const response = await AuthService.loginWithOtp(otpFormData.phone, otpFormData.otpCode, forceOverride, 'USER')
          ...
          {showConflictModal && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl animate-scaleUp">
                {conflictType === 'different_account' && (
                  <div>
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Another user is already signed in</h3>
                    <p className="text-slate-600 dark:text-slate-400 text-sm mb-4">
                      Current User: <span className="font-semibold text-primary">{currentUserEmail}</span>
                    </p>
                    <p className="text-slate-600 dark:text-slate-400 text-sm mb-6">
                      Logging in with a different account will sign out the current user. Do you want to continue?
                    </p>
                    <div className="flex flex-col sm:flex-row gap-3">
                      <button
                        onClick={async () => {
                          ...
                          if (pendingLoginAction) await pendingLoginAction();
                        }}
                      ...
                      <button
                        ...
                        className="flex-1 px-4 py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-semibold transition"
                      >
                        Stay Logged In
                      </button>
                    </div>
